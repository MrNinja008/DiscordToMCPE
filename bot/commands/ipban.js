module.exports = {
    ...
    execute: async function (m, args, config) {
        ...
        let result = await Promise.all([rcon.send("banlist ips"), rcon.send("ban-ip " + (args[1] ? "\"" + args.filter(i => args.indexOf(i) != args.length - 1).join(" ") + "\"" : args[0]) + " " + (args[args.length - 1 == 0 ? 9999 : args.length - 1] || ""))]);
        ...
        if (bannedplayers.includes(args.filter(i => args.indexOf(i) != args.length - 1 || args.indexOf(i) == 0).join(" "))) {
            m.reply("Bu IP zaten uzaklaştırılmış.");
            return;
        }
        m.reply("IP sunucudan uzaklaştırıldı.");
    }
}