        ...
        if(talepler[0] && talepler[0].ID) {
            talepler.filter(i=> i.ID.startsWith("talep_")).forEach(i=> {
                if(i.data.channelID == m.channel.id) {
                    if (client.guilds.cache.get(config.guild) && require("../index").getClient().guilds.cache.get(config.guild).channels.cache.get(i.data.channelID)) {
            ...
            Object.keys(talepler).forEach(i=> {
                if(!i.startsWith("talep_")) {
                    ...
                    if (client.guilds.cache.get(config.guild) && require("../index").getClient().guilds.cache.get(config.guild).channels.cache.get(talepler[i].channelID)) {
                        let guild = client.guilds.cache.get(config.guild);
